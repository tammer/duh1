...
def test_max_visible_scrolls_oldest_off() -> None:
    ...
    newest = ranker.consider(_cand("d", 0.65), "d", 0)
    assert newest is not None
    assert [c.term for c in ranker.visible] == ["b", "c", "d"]


def test_new_term_always_shown_when_over_cap() -> None:
    ranker = Ranker(max_visible=2)
    ranker.consider(_cand("a", 0.95), "a", 0)
    ranker.consider(_cand("b", 0.9), "b", 0)
    entered = ranker.consider(_cand("c", 0.7), "c", 0)
